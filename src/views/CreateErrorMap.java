...
public class CreateErrorMap implements ProductTable{

	private Composite container;
	private TabFolder tabfolder;
	private GridData gd = new GridData();
//	final String MicroStructure[] = {"Inheritance", "Aggregation", "SubInstance", "Creator Class", "Product Class", "Realization", "Redorectopn"};
	...
	public CreateErrorMap(Composite container,TabFolder tabfolder) {
	...
	public Table getWidget() {
		...
		for(int i = 0; i < Lole.length; i++)
		{
			...
			if	(i < 1)	col.setWidth(200);
			else		col.setWidth(150);
		}
			...
			public void mouseDown(MouseEvent e) {
				...
				for(int columNum = 0;columNum < table.getColumnCount();columNum++) {
					...
					if(item.getBounds(columNum).contains(point)) {
						...
						//createEditor(table,item,columNum);
